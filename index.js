...
|ハンドルネーム|つくぼし|
|最終学歴|早稲田大学 商学部卒|
|主要職種|ソリューションアーキテクト/ソフトウェアエンジニア|
...
\`\`\`txt
クラウドインフラのみならず、バックエンドの設計・開発経験も持ち合わせているためフルスタックな活躍が可能です。  
アーキテクチャとしては、主に Lambda や ECS Fargate 上でのバックエンドの設計・開発を行ってきました。  
言語としては主に Python 及び TypeScript を使用しており、AWS SDK や各種ライブラリを活用した開発経験があります。  
\`\`\`

### 生成AIの専門知識

\`\`\`txt
生成 AI、RAG、及び AI エージェントの分野に特化し、様々な情報を発信しています。  
...
PJに応じて、バックエンドエンジニアとしてフルスクラッチのアプリケーション開発を行ったり、生成AIエンジニアとして生成AI及びRAG関連案件の対応を行っていました。  
またプロジェクトマネージャーを兼任し、多くのPJでチームマネジメント及び顧客折衝を同時に行っていました。   
\`\`\`

以下では参画したプロジェクトの内、主要なもののみを記載します。

#### [生成AIコンピテンシー](https://classmethod.jp/news/250619-generative-ai-competency/)取得(2024/5 - 2025/6)
...
生成 AI を活用したOSSである[GenU](https://github.com/aws-samples/generative-ai-use-cases)を用いたチャットボットアプリを構築。  
...
複数の AWS アカウントに対する通知設定を一括で付与できる、通知集約システムをフルスクラッチで開発
...
SaaS から出力されるログを定期的に AWS にアップロードし、特定用途で使用するためのログ形式に加工するサーバレスアプリケーション及びCICDシステムをフルスクラッチで開発
